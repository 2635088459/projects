function least_squares_heaviside_pulse()
    % Define the function
    v = @(x) heaviside_pulse(x);
    ...
    plot_approximations(v, 'v(x) = H(x+1) - H(x-1)', domain, degrees);
    
    % Compute and display error metrics
    compute_error_metrics(v, domain, degrees);
end

function y = heaviside_pulse(x)
    y = double(abs(x) <= 1);
end
...
function plot_approximations(f, f_name, domain, degrees)
    ...
    x = linspace(domain(1), domain(2), 1001);  % Use odd number for center point
    y = f(x);
    ...
    ylim([-0.5, 1.5]);  % Adjust y-axis for better visibility
...
function coeffs = least_squares_trig(f, a, b, n)
    ...
    for i = 1:2*n+1
        ...
        if i == 1
            b(i) = integral(f, a, b);
        elseif i <= n+1
            b(i) = integral(@(x) f(x) .* cos((i-1)*pi*(x-c)/L), a, b);
        else
            b(i) = integral(@(x) f(x) .* sin((i-n-1)*pi*(x-c)/L), a, b);
        end
    end
